...
const expectedZip = `${manifest.name.toLowerCase().replaceAll(/[^a-z0-9]+/g, '-')}-${manifest.version}-chrome.zip`;
const zip = files.includes(expectedZip) ? expectedZip : undefined;
if (!zip) throw new Error(`WXT did not produce the expected extension zip: ${expectedZip}`);
